...
public class SystemBarTintManager {
    ...
    private void setupStatusBarView(Context context, ViewGroup decorViewGroup) {
        ...
        this.mStatusBarTintView.setVisibility(View.VISIBLE);
    ...
    public static void setBarsColor(Activity activity, Integer statusColor, Integer navigationColor) {
        boolean z = false;
        if (VERSION.SDK_INT >= 19) {
            setTranslucentStatus(activity, statusColor != null);
            if (navigationColor != null) {
                z = true;
            }
            setTranslucentNavigation(activity, z);
            SystemBarTintManager tintManager = new SystemBarTintManager(activity);
            if (statusColor != null) {
                tintManager.setStatusBarTintEnabled(true);
                tintManager.setStatusBarTintColor(statusColor.intValue());
            }
            if (navigationColor != null) {
                tintManager.setNavigationBarTintEnabled(true);
                tintManager.setNavigationBarTintColor(navigationColor.intValue());
            }
        }
    }
    ...
    public static void setTranslucentStatus(Activity activity, boolean on) {
        setWindowFlags(activity, 67108864, on);
    }

    @TargetApi(19)
    public static void setTranslucentNavigation(Activity activity, boolean on) {
        setWindowFlags(activity, 134217728, on);
    }

    public static void setWindowFlags(Activity activity, int flag, boolean on) {
        Window win = activity.getWindow();
        LayoutParams winParams = win.getAttributes();
        int bits = flag;
        if (on) {
